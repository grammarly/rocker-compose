...
func TestProcessConfigTemplate_Basic(t *testing.T) {
	result, err := ProcessConfigTemplate("test", strings.NewReader("this is a test {{.mykey}}"), configTemplateVars)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProcessConfigTemplate_Default(t *testing.T) {
	...
	result, err := ProcessConfigTemplate("test", strings.NewReader("this is a test {{.mykey | default \"none\"}}"), configTemplateVars)
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, "this is a test myval", result.String(), "template should be rendered")
	...
	result2, err := ProcessConfigTemplate("test", strings.NewReader("this is a test {{.mykey2 | default \"none\"}}"), configTemplateVars)
	if err != nil {
		t.Fatal(err)
	}
